...
int Mkgrp::ejecutarMkgrp(struct comando comando, ListaMontaje *listaMontaje, struct user *usuario){

    if(!usuario->active){
        std::cout<<"No hay ningun usuario logueado"<<std::endl;
        return 0;
    } else if(usuario->uid != 1){
        std::cout<<"El comando MKGRP solo puede ser utilizado por el usuario ROOT"<<std::endl;
        return 0;
    }
    ...
    //Verificar que el grupo no exista y contar la cantidad de grupos que se han creado
    ...
    for(int i = 0; i < 15; i++){
        if(inodeUsers->i_block[i] == -1){
            ...
            if(i < 12){
                if(recorrer_Directo(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_NAME].valor, groupCount, usuario, CONTAR_GRUPOS)){
                ...
                if(recorrer_Puntero_Simple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_NAME].valor, groupCount, usuario, CONTAR_GRUPOS)){
                ...
                if(recorrer_Puntero_Doble(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_NAME].valor, groupCount, usuario, CONTAR_GRUPOS)){
                ...
                if(recorrer_Puntero_Triple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_NAME].valor, groupCount, usuario, CONTAR_GRUPOS)){
    ...
    //Crear el grupo
    buffer[0] = '\0';
    sprintf(buffer, "%d", groupCount+1);
    strcat(buffer,",G,");
    strcat(buffer,parametros[T_NAME].valor);
    strcat(buffer,"\n\0");
...
int Mkgrp::recorrer_Directo(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *newGroupName, int &groupCount, struct user *usuario, Opcion dec){
            ...
            if(file->b_content[i] == '\n'){
                    ...
                    if(isGroup(buffer)){
                        ...
                        if(existGroup(buffer,newGroupName)){
                            std::cout<<"El grupo '"<<newGroupName<<"' ya existe en la particion '"<<usuario->partition_id<<"'"<<std::endl;
                            return 1;
                        }
                    }
...
int Mkgrp::recorrer_Puntero_Simple(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *newGroupName, int &groupCount, struct user *usuario, Opcion dec){
    ...
    for(int i = 0; i < 16; i ++){
        if(pointers->b_pointers[i] == -1){
            ...
            if(recorrer_Directo(pointers->b_pointers[i],superBloque,archivo,buffer,newGroupName,groupCount,usuario,dec)){
                return 1;
            }
        }
    }
...
int Mkgrp::recorrer_Puntero_Doble(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *newGroupName, int &groupCount, struct user *usuario, Opcion dec){
    //Recuperar el pointersBlock
    struct pointersBlock * pointers = new struct pointersBlock;
    fseek(archivo, superBloque->s_block_start + indice_bloque * superBloque->s_block_size,SEEK_SET);
    fread(pointers,sizeof(pointersBlock),1,archivo);
    ...
    for(int i = 0; i < 16; i ++){
        if(pointers->b_pointers[i] == -1){
            ...
            if(recorrer_Puntero_Simple(pointers->b_pointers[i],superBloque,archivo,buffer,newGroupName,groupCount,usuario,dec)){
                return 1;
            }
        }
    }
...
int Mkgrp::recorrer_Puntero_Triple(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *newGroupName, int &groupCount, struct user *usuario, Opcion dec){
    //Recuperar el pointersBlock
    struct pointersBlock * pointers = new struct pointersBlock;
    fseek(archivo, superBloque->s_block_start + indice_bloque * superBloque->s_block_size,SEEK_SET);
    fread(pointers,sizeof(pointersBlock),1,archivo);
    ...
    for(int i = 0; i < 16; i ++){
        if(pointers->b_pointers[i] == -1){
            ...
            if(recorrer_Puntero_Doble(pointers->b_pointers[i],superBloque,archivo,buffer,newGroupName,groupCount,usuario,dec)){
                return 1;
            }
        }
    }
...
int Mkgrp::crear_Bloque_Directo(int &indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer){
...
int Mkgrp::crear_Puntero_Simple(int &indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer){
...
int Mkgrp::crear_Puntero_Doble(int &indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer){
...
int Mkgrp::crear_Puntero_Triple(int &indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer){
...
int Mkgrp::esta_Lleno_Bloque_Archivo(int indice_bloque_archivo, struct superBlock *superBloque, FILE *archivo){
    struct fileBlock *file = new struct fileBlock;
...
int Mkgrp::esta_Lleno_Apuntador_Simple(int indice_bloque_punteros_simple, struct superBlock *superBloque, FILE *archivo){
    struct pointersBlock *pointers = new struct pointersBlock;
...
int Mkgrp::esta_Lleno_Apuntador_Doble(int indice_bloque_punteros_simple, struct superBlock *superBloque, FILE *archivo){
    struct pointersBlock *pointers = new struct pointersBlock;
    fseek(archivo,superBloque->s_block_start + indice_bloque_punteros_simple * superBloque->s_block_size,SEEK_SET);
    fread(pointers,sizeof(pointersBlock),1,archivo);
...
int Mkgrp::existGroup(char *buffer, char *newNameGroup){
    if(buffer[0] == '0')
        return 0;
