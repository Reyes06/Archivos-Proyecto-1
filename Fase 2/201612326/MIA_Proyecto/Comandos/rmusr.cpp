...
int Rmusr::ejecutarRmusr(struct comando comando, ListaMontaje *listaMontaje, struct user *usuario){

    if(!usuario->active){
        std::cout<<"No hay ningun usuario logueado"<<std::endl;
        return 0;
    } else if(usuario->uid != 1){
        std::cout<<"El comando RMUSR solo puede ser utilizado por el usuario ROOT"<<std::endl;
        return 0;
    }
    ...
    //Buscar el id del usuario a eliminar
    ...
    for(int i = 0; i < 15; i++){
        if(inodeUsers->i_block[i] == -1){
            ...
            if(i < 12){
                if(recorrer_Directo(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BUSCAR)){
                    encontrado = 1;
                    break;
                }
            } else if (i == 12){
                if(recorrer_Puntero_Simple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BUSCAR)){
                    encontrado = 1;
                    break;
                }
            } else if (i == 13){
                if(recorrer_Puntero_Doble(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BUSCAR)){
                    encontrado = 1;
                    break;
                }
            } else if (i == 14){
                if(recorrer_Puntero_Triple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BUSCAR)){
                    encontrado = 1;
                    break;
                }
            }
    ...
    if(!encontrado){
        ...
        fclose(archivo);
        std::cout<<"El usuario indicado no existe"<<std::endl;
        return 0;
    }
    ...
    for(int i = 0; i < 15; i++){
        if(inodeUsers->i_block[i] == -1){
            ...
            if(i < 12){
                if(recorrer_Directo(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BORRAR_GRUPO)){
                    break;
                }
            } else if (i == 12){
                if(recorrer_Puntero_Simple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BORRAR_GRUPO)){
                    break;
                }
            } else if (i == 13){
                if(recorrer_Puntero_Doble(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BORRAR_GRUPO)){
                    break;
                }
            } else if (i == 14){
                if(recorrer_Puntero_Triple(inodeUsers->i_block[i], superBloque, archivo, buffer, parametros[T_USR].valor, usuario, contador_registros, BORRAR_GRUPO)){
                    break;
                }
            }
        }
...
int Rmusr::recorrer_Directo(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *newGroupName, struct user *usuario, int &contador_registros, Opcion op){
...
int Rmusr::recorrer_Puntero_Simple(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *groupName, struct user *usuario, int &contador_registros, Opcion op){
...
int Rmusr::recorrer_Puntero_Doble(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *groupName, struct user *usuario, int &contador_registros, Opcion op){
    //Recuperar el pointersBlock
    struct pointersBlock * pointers = new struct pointersBlock;
    fseek(archivo, superBloque->s_block_start + indice_bloque * superBloque->s_block_size,SEEK_SET);
    fread(pointers,sizeof(pointersBlock),1,archivo);
...
int Rmusr::recorrer_Puntero_Triple(int indice_bloque, struct superBlock *superBloque, FILE *archivo, char *&buffer, char *groupName, struct user *usuario, int &contador_registros, Opcion op){
    //Recuperar el pointersBlock
    struct pointersBlock * pointers = new struct pointersBlock;
    fseek(archivo, superBloque->s_block_start + indice_bloque * superBloque->s_block_size,SEEK_SET);
    fread(pointers,sizeof(pointersBlock),1,archivo);
...
int Rmusr::existsUser(char *buffer, char *newNameGroup){
    if(buffer[0] == '0')
        return 0;
